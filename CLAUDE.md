...
The four regions are built (`docs/region-rebuild.md` is the brief, `docs/region-rebuild-report.md` the build report). A few old district names (Eastreena, Sunmeadow, Reedwater, Threefold) survive as landmark titles and in the opening chapter heading. Pre-rebuild versions of the rewritten files are in git history (the initial commit), not in the tree.
