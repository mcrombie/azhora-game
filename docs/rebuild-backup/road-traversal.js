...
export async function runRoadTraversal(h) {
  ...
  assert(world?.routeJourney?.length > 1, 'the northern road is missing');
  ...
  const road = world.routeJourney.map(point => ({ x: point.x, z: point.z }));
  ...
  try {
    for (let i = 0; i < road.length; i++) await walkTo(road[i], `northbound road point ${i + 1}`);

    // The visible rope is intentionally a little south of the hard map bounds.
    // Push into that real collider, then hold north for 20 stationary frames.
    const frontier = world.colliders.find(collider => collider.kind === 'frontier'
      && Math.abs(collider.x - position().x) < (collider.hx ?? collider.r ?? 0));
    const legalEdge = world.bounds.minZ + .34;
    const stopEdge = frontier ? Math.max(legalEdge, frontier.z + (frontier.hz ?? frontier.r ?? 0) + .34) : legalEdge;
    let stationaryFrames = 0, stoppedAt = null, edgeDeadline = performance.now() + 15000;
    setYaw(0); holdRun();
    while (stationaryFrames < 20) {
      assert(performance.now() < edgeDeadline, 'the northern frontier did not stop movement');
      const { current, distance, frameDelta } = await recordFrame({ allowStationary: true });
      assert(current.z >= legalEdge - .002, 'crossed the hard northern map boundary');
      if (distance < .002) {
        if (stoppedAt === null) stoppedAt = current.z;
        stationaryFrames += frameDelta;
        assert(Math.abs(current.z - stoppedAt) < .002, 'moved through the frontier while continuing to hold north');
      } else { stationaryFrames = 0; stoppedAt = null; }
    }
    assert(Math.abs(position().z - stopEdge) < .25, `stopped at ${position().z.toFixed(2)} before reaching the marked frontier ${stopEdge.toFixed(2)}`);
    traversalChecks += 2;
    stopRun();
    ...
    for (let i = road.length - 1; i >= 0; i--) await walkTo(road[i], `southbound road point ${i + 1}`);
    await walkTo(initial, 'the Eastreena starting point');
    stopRun();
    const finalState = await readState();
    const returnedToEastreena = world.regionAt(position().x, position().z).id === 1;
    assert(returnedToEastreena, 'the return journey did not re-enter Eastreena');
    assert([2, 3, 4].every(id => enteredRegions.has(id)), 'the trip skipped a northern region');
