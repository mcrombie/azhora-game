...
import { LEGACY_ROAD_TRANSFORM, compassHeading } from './region-layout.js';
...
function init() {
  ...
  const forestEcology=createForestEcology(scene,world,{exclusionSites:[...woodlandSites.acorns,...woodlandSites.sticks,...woodlandSites.fruits,...woodlandSites.fruitPatches,{x:60,z:-118,radius:12},{x:45,z:-109,radius:3.5}]});
  ...
  const meadowEncounter={id:'meadow-raiders',center:{x:15,z:-231},checkpoint:{x:8,z:-215},retreatZ:-207,enemies:[{id:'meadow-scout',x:12,z:-234,hp:65,entry:.2},{id:'meadow-scrapper',x:18,z:-237,hp:65,entry:1.5}]};
  ...
  function journalTab(tab){
    if(tab==='world')worldMap.setTraveler(LEGACY_ROAD_TRANSFORM.worldToAtlas(player.group.position.x,player.group.position.z));
  ...
  function conversation(npc) {
    ...
    if(npc.id==='harbormaster') {
      if(questStage<2) {
        ...
        lines=['You heard the bell from the boat? Bramble goblins have attacked Tidehaven. Three raiders are still on the Greenway, north of the village. The landing is safe, but the road needs help.',
          'You came for the Ambroni contract? Take this letter to Quartermaster Corvan at the Legion post on Sunmeadow Plain, just beyond our forest. He will enter you into service. Eren at the watch will show you the road.',
      ...
      else if(questStage>=6)lines=['Keep my message safe in your satchel. Follow Eren’s road north, through the forest and out to the plain. Quartermaster Corvan on Sunmeadow Plain will receive your letter and assign your Legion work.'];
      ...
      lines=['Three raiders down. Good work. Their rotten sticks made them an easier fight, but remember what kept you standing: watch the windup, dodge to the side, and counter while the stick is down. Leave yourself enough stamina to escape.',
        ...
        'Follow the north trail to Fernway Rest, then keep going until the trees give way to a broad plain. That is Eastreena’s far boundary. The open road leads into Sunmeadow Plain. Find Quartermaster Corvan at the Legion post. The Ambroni Empire hired you from abroad; he will tell you what service means here.'];
    ...
    else if(questStage>=8)lines=['Follow the cairns to Fernway Rest, and then the northern trail to Eastreena Boundary. The forest thins there and the plain opens out. Beyond the gate, the open plain begins the next leg of your journey.'];
  ...
  function testTravel(destination){
    ...
    const point=destination==='pond'?world.pond.fishingSpot:world.regions?.find(region=>String(region.id)===String(destination))?.spawn||{x:0,z:9};
  ...
  function interact(){
    ...
    if(Math.hypot(player.group.position.x-world.border.x,player.group.position.z-world.border.z)<9){
      openDialogue({id:'border-notice',name:'Eastreena Boundary',role:'Road notice'},[
        'The open gate leads to Sunmeadow Plain, then the Caloss at Reedwater Crossing, and Threefold Rise on the Luscia side. Follow the road, and greet the travelers who keep it open.',
        questStage>=10?'You have completed the first-shore tutorial. Keep Mara’s message and Eren’s token in your satchel. Corvan waits beside a stranded cart just inside Sunmeadow Plain. All four districts remain open for your return.':'Before setting out, clear the goblins, report to Eren, and check the message in your satchel. Follow the north trail through Fernway Rest to finish this leg of the journey.'
      ]);
    }
  }
  ...
  function destination() {
    ...
    if(questStage===3)return{x:0,z:-28,name:'Woodland bell'};
  ...
  function startAmbush() {
    updateQuest('ambush');combat.startEncounter();world.ringBell?.(elapsed);audio?.effect('bell');
  ...
  function handleCombatEvents() {
    for(const e of combatEvents.splice(0)) {
      ...
      if(e.type==='defeat'){
        $('defeat-checkpoint').textContent=combat.state.encounterId==='meadow-raiders'?'Full health · Restart beside the Sunmeadow field road':'Full health · Restart at the woodland bell';
  ...
  function updateHUD() {
    ...
    $('encounter-title').textContent=combat.state.encounterId==='meadow-raiders'?'THE SUNMEADOW RAIDERS':combat.state.encounterId===hideoutEncounter.id?'BRAMBLE SCOUT CAMP':'DEFEND THE GREENWAY';
  ...
  function render(now) {
    ...
    try {
      ...
      if(mode==='playing'&&!reviewFrozen) {
        ...
        if(questStage===10&&!meadowCleared&&journey.state.courierAccepted&&combat.state.phase!=='active'&&Math.hypot(player.group.position.x-15,player.group.position.z+231)<12){
          if(combat.startEncounter(meadowEncounter)){toast('Two raiders among the field walls. Give their swings room.','SUNMEADOW · WATCH THE AMBER TELLS');audio?.effect('bell');}
        }
      ...
      const {index:headingIndex,labels:headings}=compassHeading(yaw,LEGACY_ROAD_TRANSFORM);
      [...$('compass').children].slice(0,5).forEach((node,i)=>node.textContent=headings[(headingIndex+2-i+8)%8]);
      mapClock+=dt;if(mapClock>.1){updateHUD();drawMinimap(map,{world,position:player.group.position,goal:destination(),combat:combat.state,angle:player.group.rotation.y,time:elapsed,discoveries,tracked:trackedPlace(),northOffset:LEGACY_ROAD_TRANSFORM.northOffset});mapClock=0;}
  ...
  if(new URLSearchParams(location.search).has('test')) {
    ...
    const focusedRoadHooks=()=>({world,player,journey,inventory,weapons,campcraft,combat,checkpoint,journeyAct,saveRoad,continueRoad,
      ...
      prepare:()=>{questStage=10;practiceHits=2;practiceDodges=1;testingEnabled=false;inventory.grant('harbor-letter');inventory.grant('road-token');
        ...
        show('opening',false);player.group.position.set(0,world.heightAt(0,-178),-178);refreshQuest();settleCamera();},
    ...
    const localMapHooks=()=>({...forestHooks(),trailMap,localMapModel,trackPlace,clearTrailPin,trackedPlace,openLocalMap,discoverySet:discoveries,
      ...
      prepareLocalMap:()=>{forestHooks().prepareVillage();discoveries.clear();forestStory.restore();forestHideout.restore();journey.restore(createJourney().snapshot());syncForest();syncHideout();
        ...
        player.group.position.set(0,world.heightAt(0,5),5);yaw=0;pitch=.4;distance=targetDistance=9;refreshQuest();settleCamera();}});
    ...
    window.__AZHORA__={state,woodland:()=>woodlandLife.state(),roadLife:()=>roadLife.state(),roadVerges:()=>roadVerges.state(),forestEcology:()=>forestEcology.state(),forestStory:()=>forestStory.state,
      ...
      reviewRegional(view){
        ...
        let p=district===2?{x:-29,z:-273}:district===3?{x:-25,z:-448}:{x:-23,z:-573};
        yaw=.68;pitch=.42;distance=targetDistance=17;
        const focus=district===2?{x:-31,z:-278}:district===3?{x:-31,z:-452}:{x:-29,z:-577};
        ...
        if(view==='shelter-choice'){
          regionalLife.act('accept-witness-account');p={x:-32,z:-576};
      ...
      reviewLocalMap(view){
        ...
        player.group.position.set(-5,world.heightAt(-5,-108),-108);player.group.rotation.y=Math.PI;yaw=0;
        if(view==='local-nearby'||view==='local-forest-pin'){if(view==='local-nearby')player.group.position.set(0,world.heightAt(0,5),5);trackPlace('bee-fold');settleCamera();return trailMap.state();}
        openLocalMap();
      ...
      async reviewHideout(view){
        ...
        let p={x:56,z:-109};yaw=-.28;pitch=.5;distance=targetDistance=14;
        ...
        player.group.position.set(p.x,world.heightAt(p.x,p.z),p.z);player.group.rotation.y=Math.PI+yaw;
        if(view==='hideout-overview')reviewTarget=new THREE.Vector3(60,world.heightAt(60,-118)+1,-120);
        if(view==='hideout-dialogue')hideoutConversation(hideoutContext);
        if(view==='hideout-cleared'){forestHideout.inspect();forestHideout.begin({questStage});forestHideout.markCleared(hideoutEncounter.id);syncHideout();reviewTarget=new THREE.Vector3(60,world.heightAt(60,-118)+1,-120);}
      ...
      async runRecoveryCheck(){
        ...
        if(mode==='fishing')endFishing(true);
        mode='playing';player.group.position.set(12,world.heightAt(12,-232.6),-232.6);
        ...
        assert($('defeat-checkpoint').textContent.includes('Sunmeadow'),'Meadow defeat named the wrong checkpoint');
      ...
      async runTraversal(){
        ...
        player.group.position.set(0,world.heightAt(0,-158),-158);yaw=0;settleCamera();
        await frames(3);
      ...
      async runSmoke(){
        ...
        assert($('inventory-letter-body').textContent.includes('Ambroni')&&$('inventory-letter-body').textContent.includes('Sunmeadow')&&$('inventory-letter-body').textContent.includes('Bramble'),'Message contents are missing');
        ...
        yaw=0;press('KeyW');await until(()=>player.group.position.z<world.border.barrierZ-1,'Could not cross the open gate');release('KeyW');
        assert(world.regionAt(player.group.position.x,player.group.position.z).id===2,'Open gate did not enter Region 2');
        warp(world.border.x,world.border.z);await frames(2);tap('KeyF');assert(mode==='dialogue'&&$('speech').textContent.includes('Sunmeadow'),'Border notice did not explain the onward road');finishDialogue();
        ...
        tap('F8');$('test-village').click();assert(mode==='playing'&&player.group.position.z===9,'Testing village travel failed');
        ...
        for(const z of[44,32,24,19,5,-20,-40,-60])assert(canStand(0,z,world),'Main route blocked at '+z);
      ...
      review(view){
        ...
        if(view==='battle'){questStage=4;combat.startPractice(world.training);combat.finishPractice();combat.startEncounter();player.group.position.set(0,world.heightAt(0,-32),-32);yaw=.28;pitch=.32;distance=targetDistance=7;player.setArmed(true);}
        ...
        if(view==='walk'){questStage=10;combat.finishPractice();player.group.position.set(0,world.heightAt(0,-32),-32);yaw=1.15;pitch=.3;distance=targetDistance=6;player.group.rotation.y=Math.PI+yaw;}
        if(view==='inventory'){questStage=6;combat.finishPractice();inventory.grant('harbor-letter');inventory.grant('simple-sword');inventory.grant('road-token');player.group.position.set(1,world.heightAt(1,-66),-66);yaw=.2;pitch=.3;distance=targetDistance=7;toggleInventory();inventory.select('harbor-letter');}
        ...
        if(view==='traveler'){questStage=10;combat.finishPractice();player.group.position.set(0,world.heightAt(0,-15),-15);player.group.rotation.y=Math.PI;yaw=Math.PI+.35;pitch=.24;distance=targetDistance=4.5;}
        ...
        if(view==='goblin'){questStage=4;combat.finishPractice();combat.startEncounter();const enemy=combat.state.enemies[0];reviewTarget=new THREE.Vector3(enemy.x,world.heightAt(enemy.x,enemy.z)+1.15,enemy.z);reviewFrozen=true;player.group.visible=false;yaw=0;pitch=.13;distance=targetDistance=3.8;}
        if(view==='stick'){questStage=10;combat.finishPractice();inventory.grant('forest-stick');weapons.equip('forest-stick');player.group.position.set(0,world.heightAt(0,-15),-15);player.group.rotation.y=Math.PI;yaw=Math.PI+.35;pitch=.24;distance=targetDistance=4.5;}
        ...
        const roadViews={sunmeadow:{x:0,z:-248,yaw:.48,pitch:.26,distance:17},reedwater:{x:0,z:-389,yaw:0,pitch:.29,distance:14},threefold:{x:0,z:-570,yaw:-.4,pitch:.25,distance:17},'north-relay':{x:0,z:-650,yaw:.2,pitch:.25,distance:12}};
        roadViews['road-sign']={x:2,z:-380,yaw:-.15,pitch:.2,distance:5};
