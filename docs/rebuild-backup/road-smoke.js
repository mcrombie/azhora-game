...
export async function runRoadSmoke(h) {
  ...
  try {
    ...
    assert(journey.state.completedRegions.includes(2), 'Sunmeadow was not completed');
    ...
    assert(!canStand(-1.9, -412, world), 'the damaged side of the bridge was already open');
    tap('KeyF'); await frames(2);
    assert(journey.state.bridgeRepaired && world.journeySiteState()['bridge-repair'], 'bridge repair did not update the world');
    assert(canStand(-1.9, -412, world), 'repair did not make the damaged bridge lane walkable');
    ...
    await arrive(0, -380); setYaw(0);
    const bridgeStart = position.z;
    press('KeyW'); press('ShiftLeft');
    await until(() => position.z < -430, 'The repaired bridge blocked actual forward movement');
    release('KeyW'); release('ShiftLeft');
    const bridgeWalked = bridgeStart - position.z;
    assert(bridgeWalked > 50 && canStand(position.x, position.z, world), 'the bridge was not crossed on foot');
    ...
    assert(journey.state.completedRegions.includes(3), 'Reedwater was not completed');
    ...
    assert(river, 'Reedwater has no fishing bank');
