...
Status: implementation and regression checkpoint, 24 September 2026. Core flows are implemented and checked in the desktop renderer; broader regression history and remaining extensions are listed below. The shared story model, physical route driver, host integration, Republican recruitment and common Solis entry are implemented. Ben's F8 quest autoplay is a separate completed feature. The sections below preserve the approved design; implementation details and verification are recorded at the end.
...
At the 23 September checkpoint, the full historical `npm test` run recorded 1,639 passes and 51 failures before the final fixes. A newly exposed shared-Luscia save validation bug was fixed and its focused regression tests passed. Several failures were assertions tied to obsolete source shapes or removed scene content; others concerned road/scenery geometry requiring investigation. A subsequent general desktop smoke run passed the updated shared-satchel dialogue checks but stopped at its collision assertion. That failure was traced to Willowmere pond using sea level instead of its own surface; the water marker was fixed and the exact first-50-collider assertion plus bank-to-pond swimming passed in a focused real-world regression. The following resumed-validation section records the newer results. Neither checkpoint is a claim to have played every route from beginning to end or to have an all-green historical full suite.
...
### Resumed validation, 24 September

The longer world-route simulation now runs all ten mercenaries through the living main story against the built world's real colliders. In its deterministic scenario, Chris dies without assistance, nine survivors reach their faction musters, the company repairs the bridge once and delivers the satchel once, Republican defections retain their actors, and only four horses are claimed. It reloads during the repair, checks paused work, and selects the first loyal Imperial courier when Ed defects. Encounter outcomes are supplied by the simulation host; this does not claim a full visual combat playthrough.

Resolved road obstructions include the repair bench near Sava's shrine, curved approaches into the Moros outpost gates, and missing navigation streets within Solis. Navigation now samples actual distances along the road curve. A short joining segment at Reedcutters Camp also caused autoplay to return repeatedly to an endpoint behind the player; steering now follows the joining leg. The final targeted navigation/autoplay runs passed 93 tests, including the unchanged ambush-bearing assertion, mounted road clearances, and actual Solis and Reedcutters routes.

The live autoplay adapter now supplies the saved campaign entry and the Republican soldier's death state. An early recall therefore continues at the muster after reloading, and the planner does not seek a dead Davin. The native living-story suite passed 89 assertions with zero frame errors, including that real adapter boundary. Chapter 1's native checks passed training, the required chart lesson, a visible and winnable ambush, optional silver tracking, swimming the unrepaired Caloss, Nothom reporting without Chip, and the separate carpentry lesson. The companion check waits for the actual next-frame route update.

The journal's Company page had been trapped inside the hidden legacy Journey content. It now has its own tab and displays the selected hero's actual roster, current activities and deaths. Native journal checks passed 26 assertions plus compact layout checks for Journey, Skills and Company at 900 by 700; the Company page was visually reviewed at compact and full sizes. Seventy additional host/companion regressions passed after replacing obsolete timetable/source-text assumptions with execution of the actual host boundaries.

General smoke-test review views use separate goblin identities, so composing a screenshot after a completed fight does not try to resurrect the real dead actors. Ordinary corpse persistence is unchanged. This fixes review setup rather than weakening combat assertions.

The final focused `npm run test:living` run passes all 217 tests. The full general desktop smoke also passes, including its gameplay checks and subsequent review captures, with zero frame errors and no recorded errors. It covers 150 road checks, the shared satchel and claimed horse, 50 collision checks, the chart, inventory, combat, and woodland activities. The web build also succeeds.

The 1,639/51 historical full-suite result above is a baseline from before these corrections, not a current failure count. The complete historical suite has not been rerun during this resumed pass. Keep the remaining design and transport limits above explicit when extending the campaign.
