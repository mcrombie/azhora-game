...
test('Elod’s frontier guards wear light black armour: lamellar, a hood under an open helm, soft boots, a short spear or a bow and a small round shield', () => {
  ...
  for (const actor of [spear, bow, captain]) {
    ...
    for (const name of ['Legion banded cuirass', 'Legion helmet', 'Legion shield', 'Suvali studded jerkin', 'Suvali iron cap'])
      assert.equal(actor.group.getObjectByName(name), undefined, `does not wear ${name}`);
    const { draws, triangles, red, black } = inspect(actor);
    assert.equal(red, 0, 'nothing of the Legion’s red');
...
test('an Elodi guard is leaner and a little taller than a legionary, and his spear is shorter', () => {
  ...
  assert.ok(height(guard, 'Elodi short spear') < height(legionary, 'Legion spear'));
});
