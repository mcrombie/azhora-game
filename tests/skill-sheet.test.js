...
test('fourteen skills, fourteen marks, drawn the way the satchel draws its items', () => {
  assert.equal(SKILL_IDS.length, 14, 'the ten skills, cartography, swimming, farming and the linguist');
...
test('the sheet is a grid of tiles, with each skill’s guide and log behind its own tile', () => {
  ...
  assert.match(css, /#skills-sheet \.skill-tile-total\{grid-column:span 1/, 'and a total that fills what the fourteenth skill leaves of the last row');
