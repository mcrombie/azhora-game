...
test('the tutorial advances only through the tab each step asks for, chart first and trails second', () => {
  ...
  assert.equal(tutorial.view().card.id, 'trails');
  assert.equal(tutorial.noteJournalTab('world'), false, 'reopening the chart does not skip the trails');
  assert.equal(tutorial.noteJournalTab('trails'), true);
...
test('snapshots round-trip and invalid saved steps are refused without changing the tutorial', () => {
  ...
  for (const good of [0, 1, 2, 3]) {
    ...
    assert.equal(tutorial.snapshot(), good);
  }
  ...
  assert.deepEqual(MAP_TUTORIAL_STEPS.slice(1).map(card => card.act), ['open-chart', 'open-trails', null], 'the autopilot knows what each step asks for');
});
