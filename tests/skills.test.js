...
test('levels are read from the thresholds, with progress toward the next', () => {
  // Thirteen that are about the world, then the seven that are about fighting, which sit under
  ...
  assert.ok(SKILL_IDS.slice(0, 13).every(id => SKILLS[id].group === undefined), 'and nothing else is grouped');
