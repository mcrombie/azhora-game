...
test('local destinations stand on their own authored regions, with a separate schematic route', () => {
  ...
  assert.deepEqual(locals.map(destination => destination.regionId), [...PLAYABLE_REGIONS], 'one stop per playable region, in order');
  assert.deepEqual(locals.map(destination => destination.region), PLAYABLE_REGIONS.map(name => REGION_IDS[name]));
