...
test('Ansel’s red-tail rides his glove, goes up to circle the green, and glides back down to his fist', () => {
  ...
  assert.equal(step.yaw, glove.yaw, 'facing the way Ansel faces');
  ...
  assert.ok(Math.min(...radii) > RED_TAIL.radius[0] - .1 && Math.max(...radii) < RED_TAIL.radius[1] + .1, 'round Ansel, not off across the country');
...
test('while the traveler talks with Ansel she comes in early and stays on the glove', () => {
  const hawk = createHawkFlight({ random: sequence([.5]) });
...
test('the hawk is a half-metre raptor with a red tail, folded on the fist and a metre and more across in the air', async () => {
  const { createRedTailHawk } = await sourceModule('../src/ansel-hawk.js');
...
test('Ansel is drawn from the sketch, holds his fist up for her, and talks about her', async () => {
  const { createCharacter } = await sourceModule('../src/characters.js');
  const ansel = createCharacter({ role: BIRD_WATCHER.modelRole, tunic: BIRD_WATCHER.color });
  const wristHeight = pose => {
    for (let i = 0; i < 90; i++) ansel.animate(i / 30, 0, true, pose);
    ansel.group.updateMatrixWorld(true);
    return ansel.group.getObjectByName('Left Wrist').getWorldPosition(new THREE.Vector3()).y;
  };
