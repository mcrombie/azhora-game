...
if (ownsInstance) app.whenReady().then(async () => {
  ...
  win.webContents.on('console-message', (_, level, message) => { if (level >= 3) errors.push(message); if ((cagneyAutoplayChecksOnly && message.startsWith('CAGNEY_AUTOPLAY_PROGRESS'))||(bearQuestChecksOnly&&message.startsWith('BEAR_'))) console.log(message); if(bearQuestChecksOnly&&message.startsWith('BEAR_CAPTURE ')){const name=message.slice(13).replace(/[^a-z0-9-]/gi,'');win.webContents.capturePage().then(img=>fs.writeFileSync(path.join(__dirname,'tests/artifacts','bear-'+name+'.png'),img.toPNG()));} });
