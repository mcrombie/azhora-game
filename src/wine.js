 ...
 * Luscia, southeast of Port Calos.
 * KAT pours there; every wine tasted properly for the first time
 * teaches something.
 *
 ...
 *
 * Once the traveler has tasted four of the eight, KAT opens the cellar: five
 ...
 * which arrive as it levels, and MAT's walk through the cellar (`CELLAR_WALK`),
...
export const WINES = Object.freeze({
  viognier: wine('viognier', { xp: 15,
    ...
    lore: 'The white this country is proudest of. It is fussy in the vineyard and generous in the glass, which KAT says describes most of her family.' }),
  ...
  merlot: wine('merlot', { xp: 10,
    ...
    lore: 'The easy red, the one KAT pours for soldiers. It forgives a cold year better than it forgives a wet one.' }),
  'petit-verdot': wine('petit-verdot', { xp: 15,
    ...
    lore: 'Elsewhere it goes into a blend a spoonful at a time. Here it ripens fully, and KAT bottles it on its own out of pride.' }),
  tannat: wine('tannat', { xp: 20,
    ...
    lore: 'Named, they say, for the very thing it has most of. Five years in the barrel before KAT will pour it for anyone, and she still warns them.' }),
 ...
 * KAT keeps these back until somebody has tasted four of the plain wines: they are easier
...
export const CELLAR_WINES = Object.freeze({
  'sparkling-chardonnay': cellarWine('sparkling-chardonnay', { name: 'Bottle-fermented Chardonnay', colour: 'white', from: 'chardonnay', xp: 25,
    ...
    lore: 'Every bottle is turned a little further onto its nose by hand, a few days at a time, until the dregs collect in the neck. Then the neck is frozen, the cap comes off, and the plug of ice fires out. MAT can do sixty in an hour and will show you if you stand still.' }),
  'cabernet-franc-rose': cellarWine('cabernet-franc-rose', { name: 'Cabernet Franc rosé', colour: 'rosé', from: 'cabernet-franc', xp: 20,
    ...
    lore: 'KAT makes it because the pickers asked for something to drink at noon that would not put them to sleep. It is the first thing to sell out every year, which she says is a judgement on her serious wines.' }),
  'vidal-ice': cellarWine('vidal-ice', { name: 'Vidal picked frozen', colour: 'sweet', from: 'vidal-blanc', xp: 25,
    ...
    lore: 'It has to be picked in the dark, before the sun gets at it, and the whole valley turns out for it with lanterns and gloves. MAT says it is the only night of the year the work is a party.' }),
  'amber-viognier': cellarWine('amber-viognier', { name: 'Viognier left on its skins', colour: 'amber', from: 'viognier', xp: 25,
    ...
    lore: 'The oldest way there is, and the one people find strangest now. KAT keeps two jars buried to the neck in the hall floor, and argues with every merchant who calls it spoiled.' }),
  'fortified-norton': cellarWine('fortified-norton', { name: 'Fortified Norton', colour: 'red', from: 'norton', xp: 30,
    ...
    lore: 'A winter wine, and the one KAT pours for anybody who has walked up the lane in the rain. There is a barrel of it started before she was born that gets topped up and never emptied.' }),
});
export const CELLAR_WINE_IDS = Object.freeze(Object.keys(CELLAR_WINES));
/** How many of the plain eight must be tasted before KAT opens the cellar. */
...
/** MAT's walk through the hall, in the order it happens to the grape. */
export const CELLAR_WALK = Object.freeze([
  ...
  'Then the second change, the quiet one: the sharp apple acid turns to the soft milk acid. We let it happen to everything red and to whichever whites KAT wants round rather than sharp.',
  ...
  'And bottling, which is the only part that has to be done fast, in one day, with everybody in the valley pressed into it and KAT counting corks. Then you leave it alone, which is the hardest work in the cellar.',
]);
...
export function createWine({ skills, onEvent = () => {} } = {}) {
  ...
  /** Learned from Lakota (who also sends the traveler to the winery) or from KAT at the winery. */
  ...
  /** Reaching the winery and meeting KAT. */
  function visit() {
  ...
  function taste(id) {
    ...
    // KAT's eight, the five she makes from them in the cellar, or Juan's eight at Tharganhom in Solis.
  ...
  /** How many of the plain eight have been tasted: KAT opens the cellar at four. */
  ...
  function view() {
    ...
    const cellar = CELLAR_WINE_IDS.map(id => ({ id, tasted: !!state.tasted[id], colour: CELLAR_WINES[id].colour,
      name: state.tasted[id] ? CELLAR_WINES[id].name : `Something of KAT’s made from the ${WINES[CELLAR_WINES[id].from].name}`,
      detail: state.tasted[id] ? CELLAR_WINES[id].note
        : cellarOpen() ? 'KAT has offered it from the cellar.' : `Taste ${CELLAR_OPENS_AT} of the eight first; then KAT opens the cellar.` }));
    return { met: state.met, quest: state.quest, tastedCount: tastedCount(), total: WINE_IDS.length + CELLAR_WINE_IDS.length + ATTIC_WINE_IDS.length,
      ...
      entries: [...WINE_IDS.map(id => ({ id, tasted: !!state.tasted[id], colour: WINES[id].colour,
        ...
        detail: state.tasted[id] ? WINES[id].note : state.met ? `KAT pours it at ${WINERY.name}.` : 'Lakota, the man on the road with the hawk, knows wine.' })), ...cellar, ...attic] };
  }
...
/** KAT at the cabin porch. `act` runs 'learn-wine-here' and 'taste-<id>' in the host. */
export function vintnerConversation(npc, context) {
  const { wine, openDialogue, closeDialogue, act } = context;
  if (![WINEMAKER.id, CELLAR_HAND.id].includes(npc.id)) return false;
...
export function winemakerConversation(npc, context) {
  ...
  const barrel = hunt && katy?.looking && !hunt.has('vial') ? [{ id: 'kat-barrel', label: 'Has anything odd come back off the war?',
    action: () => openDialogue(npc, [
      ...
      'Take it. KAT does not know and I would rather she went on not knowing until somebody can tell her what it is. You are the one wandering about with a drawing of a monster in your satchel, so. You take it.',
...
/** MAT Arrend among the barrels, who will walk anybody who asks through the whole making of it. */
export function cellarHandConversation(npc, context) {
  ...
  openDialogue(npc, [
    ...
    'KAT pours at the cabin. Ask her for the Norton. Nobody asks for the Norton, and she likes the ones who do.',
...
/** Wine knowledge has several independent teachers and no Farming prerequisite. */
export const WINE_INTRO_TEACHERS = Object.freeze([
  'winemaker', 'cellar-hand', 'ben-sorcerer', 'liz-beekeeper', 'bee-keeper',
]);
export function wineIntroChoice(npc, {wine,teach} = {}) {
  if (!WINE_INTRO_TEACHERS.includes(npc?.id) || wine?.met) return null;
  return {id:'learn-wine-here',label:'Can you teach me about wine?',action:()=>teach(npc)};
}
