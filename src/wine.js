 ...
 * learned it at Paradise Springs (src/winery.js) and teaches the first of it in
 * Tidehaven, then sends the traveler to the winery itself, in the north-east
 * of West Suval, with a warning about the war around Solis. Livia Seravo pours
 * there; every wine tasted properly for the first time teaches something.
 *
 * The wines are the wines of the country the game is drawn from: Viognier, the
 * white it is proudest of; Petit Manseng, sharp enough to sweeten; Chardonnay
 * from the barrel; a dry rosé of Cabernet Franc; Cabernet Franc itself, the red
 * that suits the place; Petit Verdot, inky and grippy; and Norton, the native
 * vine that grew here before anybody planted one.
 * Pure: no DOM, no three.
 */
import { VINTNER, CELLAR_HAND, WINERY } from './winery.js';

export const WINE_VERSION = 1;
export const WINE_SKILL = 'wine';
export const VISIT_XP = 25;

const wine = (id, entry) => Object.freeze({ id, ...entry });
export const WINES = Object.freeze({
  viognier: wine('viognier', { name: 'Viognier', colour: 'white', xp: 15,
  ...
  'petit-manseng': wine('petit-manseng', { name: 'Petit Manseng', colour: 'white', xp: 15,
    note: 'Deeper gold. Pineapple, ginger, a streak of honey, and an acid so bright it makes your jaw ache. Off-dry, and it can take the sugar.',
    lore: 'Thick skins that shrug off the autumn rain. Left on the vine late it makes a sweet wine to keep for twenty years.' }),
  chardonnay: wine('chardonnay', { name: 'Chardonnay', colour: 'white', xp: 10,
  ...
  rose: wine('rose', { name: 'Rosé of Cabernet Franc', colour: 'rosé', xp: 10,
    note: 'Onion-skin pink. Strawberry and watermelon, bone dry, a wine for the terrace in the afternoon.',
    lore: 'Red grapes pressed quickly, before the skins can colour it more than a blush. The first wine of the year to be ready.' }),
  'cabernet-franc': wine('cabernet-franc', { name: 'Cabernet Franc', colour: 'red', xp: 15,
  ...
  'petit-verdot': wine('petit-verdot', { name: 'Petit Verdot', colour: 'red', xp: 15,
  ...
  norton: wine('norton', { name: 'Norton', colour: 'red', xp: 20,
...
export function createWine({ skills, onEvent = () => {} } = {}) {
  ...
  /** Reaching Paradise Springs and meeting Livia. */
  function visit() {
  ...
  function task() {
    ...
    return { title: 'Paradise Springs', stage: 'visit', target: VINTNER.id,
      detail: 'Lakota’s old winery, in the north-east of West Suval: take the lane east off the Solis road past the Suval Downs. There is a war around Solis. Keep your head down.' };
  }
  function view() {
    return { met: state.met, quest: state.quest, tastedCount: tastedCount(), total: WINE_IDS.length, task: task(),
      entries: WINE_IDS.map(id => ({ id, tasted: !!state.tasted[id], colour: WINES[id].colour,
        name: state.tasted[id] ? WINES[id].name : `A ${WINES[id].colour} of Paradise Springs`,
        detail: state.tasted[id] ? WINES[id].note : state.met ? 'Livia pours it at Paradise Springs.' : 'Lakota, the birder in Tidehaven, knows wine.' })) };
  }
...
export function vintnerConversation(npc, context) {
  ...
  const opening = firstVisit
    ...
    `This is ${WINERY.name}. The cabin is the first house anybody built on this land; we pour in it now. The hall is where the wine is made, and the spring below is why anyone settled here at all.`]
