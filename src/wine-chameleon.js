 ...
 * things under the sea wall, the springs beneath the town), and Ed's magic
 * keeps them from quarrelling, but only while it runs loose, which is to say
 * only while he is drunk. The old kings kept a Cup-Bearer to the Chameleon. The
 * Empire abolished the office with everything else it did not understand. Now
 * Prime Minister Leandro Dorsael pays for Ed's wine from a line in the harbour
 * accounts, and his secretary leaves it for the chameleon in a niche in the sea wall.
 * If Solis learned that its taxes keep Ed in wine, he would be finished by
 * supper.
 *
...
export const ED = freeze({ id: 'ed', name: 'Ed', role: 'The wine chameleon of Solis', stay: 95, spook: 2.6, swingSpook: 3.6, talk: 3.4 });
...
/** Where Ed turns up. On the ridge of Tharganhom's roof he is out of reach, and knows it. */
export const ED_HAUNTS = freeze([
...
export const ED_HAUNT_IDS = freeze(ED_HAUNTS.map(haunt => haunt.id));
/**
...
/** What the city feels as Ed sobers, once the deliveries have stopped. */
export const SOBER_SIGNS = freeze([
  ...
  freeze({ at: 1, title: 'THE ORANGE COURT', line: 'Every tree in the orange court has dropped its fruit, green. Ed has been sober for a whole day.' }),
]);
/** Seconds of play for a sober Ed to go from the first sign to the last. */
...
export function validateEdSnapshot(data, { allowMissing = true } = {}) {
  if (data === undefined) return allowMissing;
  if (!data || typeof data !== 'object' || Array.isArray(data) || data.version !== 1) return false;
  if (!ED_HAUNT_IDS.includes(data.haunt) || typeof data.met !== 'boolean' || !QUESTS.includes(data.quest)) return false;
...
export function createEd({ random = Math.random } = {}) {
  const state = { haunt: 0, clock: 0, met: false, quest: 'none', gifts: 0, poofs: 0, sober: 0, signs: 0 };
  const haunt = () => ED_HAUNTS[state.haunt];

  function poof(reason) {
    const from = haunt();
    let next = Math.floor(random() * (ED_HAUNTS.length - 1));
  ...
  function update(dt, traveler = null) {
    ...
    if (traveler && ((traveler.hurrying && d < ED.spook) || (traveler.swinging && d < ED.swingSpook))) events.push(poof(traveler.swinging ? 'swung-at' : 'chased'));
    // Out of reach on the ridge, he moves on when he likes; on the ground he stays while somebody is with him.
    else if (state.clock > ED.stay && (d > 12 || here.perch)) events.push(poof('wandered'));
  ...
  /** A bottle for Ed, from the traveler's satchel: he drinks it, and whatever had started to go wrong settles. */
  function feed(item) {
    const id = ATTIC_BOTTLES[item];
    if (!id) return { ok: false, reason: 'He sniffs it and pushes it back at you with his tail. “Wine, long-legs. Wine.”' };
    state.gifts++;
  ...
  function task() {
    const q = state.quest;
    if (q === 'heard') return { title: 'Ed, the wine chameleon', stage: 'heard', detail: 'Everybody in Solis hates Ed, and nobody can catch him. Juan says the chameleon has drunk half his stock. And yet Ed has never once gone thirsty. Somebody is feeding him. Ask about the city; watch where he drinks.' };
    if (q === 'cask') return { title: 'Ed, the wine chameleon', stage: 'cask', target: SECRETARY.id, detail: 'A cask of good wine in a niche in the sea wall, sealed in green wax with a sun-horse over a ledger: the seal of the Prime Minister’s office. His secretary keeps the door of the old counting house.' };
    if (q === 'told') return { title: 'Ed, the wine chameleon', stage: 'decide', target: SECRETARY.id, detail: `${PRIME_MINISTER} keeps Ed in wine to keep the old spirits of Solis at peace, and it would end him if the city knew. Tancredi Vel is waiting to hear what you will do.` };
    return null;
  }
  ...
  function restore(data) {
    ...
    if (!validateEdSnapshot(data, { allowMissing: false })) return false;
    Object.assign(state, { haunt: ED_HAUNT_IDS.indexOf(data.haunt), met: data.met, quest: data.quest, gifts: data.gifts, poofs: data.poofs, sober: data.sober, signs: data.signs });
    return true;
  }
...
const ED_TALK = freeze({
  ...
  hated: [
    ...
    'And yet: fed. Funny, that. The most hated chameleon in Solis, and never once gone thirsty. Think about it, long-legs. No, do not. Thinking is how it starts.',
  ],
});
...
export function edConversation(npc, context) {
  ...
  openDialogue(npc, opening, null, 'Leave him be', { choices: [
    ...
    ...(bottles.length ? [{ id: 'ed-gift', label: 'Here. Have a bottle.', action: () => openDialogue(npc, ['His eyes go very wide and very bright.'], null, 'Back to Ed', { choices: [
      ...
      { id: 'ed-gift-none', label: 'On second thought, no.', action: again },
    ] }) }] : []),
    { id: 'ed-grab', label: 'Grab him!', action: () => { closeDialogue(); act('ed-grab'); } },
    { id: 'leave-ed', label: 'Good night, Ed.', action: closeDialogue },
  ] });
  return true;
}
/** What he says to a gift, drunk again. */
export const edThanks = (wine, settled) => [
  `For me? For ME? Oh, you beautiful long-legged thing. The ${wine.name}!`,
  'He drinks half of it in one go, sighs like a kettle, and the colour comes back into him: green, then gold, then a deep, contented purple.',
  ...(settled ? ['Somewhere down by the quay, something under the water goes quiet.'] : ['A rhyme for you, then: / The sea is calm, the hills are kind, / and Ed is drunk. Now never mind.']),
];
...
const SECRETARY_TRUTH = freeze([
  ...
  'Keep your voice down. Yes, it is his seal. Yes, the cask is for the chameleon. No, he is not mad.',
  'Solis stands where three old things meet: the spirits of the vines on the hills, the things under the sea wall, and the springs under the town. They have never liked each other. Before the walls and before the kings, something had to sit in the middle and keep them from quarrelling. That something is Ed.',
  ...
  'The old kings had an office for it: Cup-Bearer to the Chameleon, with a salary and a seal and a seat at the feasts. The Empire abolished it with everything else it did not understand, and we had the winter of the green oranges. Now the Prime Minister pays for his wine out of the harbour accounts, under sundries, and I carry it to the sea wall every tenth night.',
  `You have seen what this city thinks of Ed. He drank the wedding wine at half the weddings in Solis. If it came out that the city’s money keeps him in wine, ${PRIME_MINISTER} would be finished by supper, and whoever came next would stop the deliveries out of pure embarrassment. So. What are you going to do?`,
]);
/** The secretary. `act` runs 'ed-confront', 'ed-keep' and 'ed-expose'. */
export function secretaryConversation(npc, context) {
  const { ed, openDialogue, closeDialogue, act } = context;
  if (npc.id !== SECRETARY.id) return false;
  const q = ed.quest;
  const decide = lines => openDialogue(npc, lines, null, 'Leave the counting house', { choices: [
    { id: 'ed-keep', label: 'Your secret is safe with me.', action: () => { closeDialogue(); act('ed-keep'); } },
    { id: 'ed-expose-ask', label: 'The city has a right to know.', action: () => openDialogue(npc, [
      ...
      { id: 'ed-expose', label: 'Tell them anyway.', action: () => { closeDialogue(); act('ed-expose'); } },
      { id: 'ed-expose-no', label: 'No. Keep it.', action: () => { closeDialogue(); act('ed-keep'); } },
    ] }) },
  ] });
  if (q === 'cask') {
    openDialogue(npc, [`${SECRETARY.name}, private secretary to ${PRIME_MINISTER}. The Prime Minister is not receiving.`], null, 'Leave the counting house', { choices: [
      { id: 'ed-show-seal', label: 'I found a cask in the sea wall. Sealed with a sun-horse over a ledger.', action: () => { act('ed-confront'); decide([...SECRETARY_TRUTH]); } },
  ...
  else if (q === 'exposed') openDialogue(npc, [`${PRIME_MINISTER} resigned this morning. His successor has stopped the deliveries, as I said she would.`, 'Listen to the sea wall tonight, and then tell me the city had a right to know. If you want to put it right, you know where the chameleon drinks.'], null, 'Leave the counting house');
