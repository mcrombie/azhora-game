...
/**
 * The rough chart Tidehaven keeps, which Mara hands over on the landing. The coast from Feradom
...
export const STARTING_CHART = Object.freeze({
  Drent: Object.freeze({ state: 'charted', named: true }),
  Feradom: Object.freeze({ state: 'charted', named: false }),
  Pueth: Object.freeze({ state: 'charted', named: false }),
  Luscia: Object.freeze({ state: 'charted', named: false }),
  'East Suval': Object.freeze({ state: 'charted', named: false }),
  'West Suval': Object.freeze({ state: 'charted', named: false }),
});
...
export function createCartography({ skills = null, onEvent = () => {} } = {}) {
  ...
  /** Mara's rough chart, and the lesson that goes with it. Learned once. */
  function learn() {
  ...
  function noteHex(name) {
    ...
    if (first) {
      ...
      it.named = true;
      result = { ...result, xp: CHART_XP.firstHex, state: it.state, named: true, ...paid };
      onEvent({ type: 'chart-changed', region: name, state: it.state, named: true, xp: CHART_XP.firstHex, ...paid });
    }
