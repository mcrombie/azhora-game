...
function init() {
  ...
  world.keepPropsClear([...Object.values(world.npcPositions),...SALT_PORTS.map(p=>p.stand),...FOREST_STORY_SITES,...REGIONAL_LIFE_SITES,...Object.values(LUSCIA_SITES),...Object.values(MOROS_SITES)]);
  ...
  const wood=createWoodcutting({skills});
  ...
  const edContext=()=>({ed,salt,inventory,openDialogue,closeDialogue,act:edAct});
  ...
  function conversation(npc) {
    ...
    if(npc.id===BOWDEN.id){wood.visit();bowdenConversation(npc,{wood,skills,purse:inventory.count(COPPER_ITEM),count:id=>inventory.count(id),has:id=>inventory.has(id),openDialogue,closeDialogue,act:woodAct});return;}
