...
import { LUMBER_TOWN_STABLE, SOLIS } from './region-world.js';
...
function init() {
  ...
  for(const npc of npcData) {
    npc.actor=npc.ogre?createOgre():npc.dog?createDog({variant:0}):npc.cat?createCat({variant:0}):createCharacter({tunic:npc.color,role:npc.modelRole||npc.id,skin:npc.skin,look:npc.look,armed:!!npc.armed});const p=world.npcPositions[npc.id];if(npc.hidden)npc.actor.group.visible=false;
  ...
  const archaeology=createArchaeology({skills}),wine=createWine({skills}),cooking=createCooking({skills}),wineAttic=createWineAttic(),digs=createRenaDigs(scene,world);let currentDig=null,currentVine=null;
  ...
  const atticContext=(extra={})=>({attic:wineAttic,wine,purse:inventory.count(COPPER_ITEM),items:INVENTORY_ITEMS,openDialogue,closeDialogue,act:atticAct,...extra});
  ...
  function gatherBodies(){
    ...
    for(const npc of npcData){if(npc.hidden||npc.fallen||!npc.actor.group.visible)continue;const p=npc.actor.group.position;list.push({id:npc.id,x:p.x,z:p.z,r:npc.cat?BODY.cat:npc.dog?BODY.dog:npc.ogre?BODY.ogre:BODY.person});}
  ...
  function localMapKnown(){
    ...
    if(discoveries.has('village'))for(const id of ['fisher','acorn-cook','doomsayer','forest-woodcutter',BIRD_WATCHER.id])knownNPCs.add(id);
  ...
  function dodge(){
    if(mode!=='playing'||!grounded||riding.mounted)return;
    ...
    try {
      ...
      if(mode==='playing'&&!reviewFrozen) {
        ...
        if(riding.mounted){
          ...
          moveCharacter(player.group.position,dx,dz,playerWorld);
