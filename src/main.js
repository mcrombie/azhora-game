...
function init() {
  ...
  function localMapKnown(){
    ...
    if(questStage>=1)knownNPCs.add(BIRD_WATCHER.id);
  ...
  function conversation(npc) {
    ...
    if(npc.id===JOHN.id){salt.visit();johnConversation(npc,{salt,hunt,openDialogue,closeDialogue,act:saltAct});return;}
