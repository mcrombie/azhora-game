/**
 * Puck, the wine goblin of Solis: clever, magic, permanently drunk, never
 * without his sunglasses, and never without his pipe.
 *
 * He was Ed until this commit, and Ed was two creatures at once: the one who
 * holds Solis together and the one whose whole charm is that he is never where
 * you left him. Puck keeps the city, the sea wall, the Prime Minister's cask
 * and every line of this file. Ed keeps the chameleon's body and the vanishing,
 * and is off being a chameleon somewhere else entirely (`src/chameleon.js`).
...
export function createPuck({ random = Math.random } = {}) {
  ...
  function feed(item) {
    ...
    if (!id) return { ok: false, reason: 'He sniffs it and pushes it back at you with one long grey finger. “Wine, long-legs. Wine.”' };
    state.gifts++;
...
const PUCK_FIRST = freeze([
  ...
  'He lifts, in both small grey hands, a bottle that is certainly not his. “Your health! Or mine. I am not particular. Hic.”',
...
/** Puck on his other job: Chief Taster to John, the Sultan of the Salt Trade (src/salt-sultan.js). */
const PUCK_JOHN = freeze([
  'Chief Taster to the Sultan of Salt! / I lick, I judge, I call a halt.',
  'He wets a finger, touches the wall, and tastes it. “Saltings white, the south pans, third week of summer. The man who raked it was in love. Hic.”',
  'It is a terrible job for a goblin. Salt makes you thirsty. That is, in fact, why I took it.',
...
const PUCK_JOHN_SOBER = freeze(['“John. Yes. I taste his salt for him. I can’t, just now. Everything tastes of nothing.”']);
const PUCK_SULTAN_IN = 'The Sultan’s in! The Sultan’s here! / The accounts are… somewhere. Somewhere near.';
const PUCK_TALK = freeze({
  who: [
    ...
    'The pipe? Drent leaf, the good stuff, off a drying rack in Tidehaven that I have never been anywhere near. A goblin has two vices, long-legs: the one in the bottle and the one in the bowl. And the glasses. Three. Three vices.',
  ],
  why: [
 ...
 * and whether John is in Solis. `act` runs 'puck-meet', 'puck-grab' and 'puck-gift-<item>'.
 */
export function puckConversation(npc, context) {
  const { puck, salt = null, inventory = null, openDialogue, closeDialogue, act } = context;
  ...
  if (!sober && !first && !context.back && salt?.inPort?.('solis')) opening.unshift(PUCK_SULTAN_IN);
  ...
  openDialogue(npc, opening, null, 'Leave him be', { choices: [
    ...
    ...(salt?.met ? [{ id: 'puck-john', label: 'John says you’re his Chief Taster.', action: () => talk(sober ? PUCK_JOHN_SOBER : PUCK_JOHN) }] : []),
...
export const puckThanks = (wine, settled) => [
  ...
  'He drinks half of it in one go, sighs like a kettle, and the grey goes out of him: green again, then a deep, contented purple about the ears.',
